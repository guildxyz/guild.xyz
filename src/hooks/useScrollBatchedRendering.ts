...
export function useScrollBatchedRendering(props: CommonProps): MutableRefObject<null | HTMLElement>
...
export function useScrollBatchedRendering({
  ...
  useScrollEffect(() => {
    ...
    if (
      !target.current ||
      target.current.getBoundingClientRect().bottom - offsetPixel > window.innerHeight ||
      disableRendering
    ) {
      return
    }
