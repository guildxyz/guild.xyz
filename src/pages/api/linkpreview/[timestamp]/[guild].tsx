...
const ErrorContainer = ({ children }) => (
  ...
  >
    ...
    >
      <div style={{ display: "flex", fontSize: "6rem", marginBottom: "0.3em" }}>
        <NotFoundIcon />
      </div>
      {children}
    </div>
...
const handler = async (req, _) => {
  ...
  if (!interFontData || !interBoldFontData || !interFont) {
    return new ImageResponse(
      (
        <ErrorContainer>
          <div style={{ fontSize: "1.125rem" }}>
            Uh-oh! Something went wrong, please contact us on our Discord server if
            you think you shouldn't see this page!
          </div>
        </ErrorContainer>
      ),
      {
  ...
  const [guild, guildRoles, hasError]: [Guild, Guild["roles"], boolean?] =
    await Promise.all([
      fetcher(`/v2/guilds/${urlName}`),
      fetcher(`/v2/guilds/${urlName}/roles`),
    ]).catch(() => [null, null, true])

  if (hasError)
    return new ImageResponse(
      (
        <ErrorContainer>
          <div style={{ fontSize: "6rem", lineHeight: 0.8, marginBottom: "0.23em" }}>
            404
          </div>
          <div style={{ fontSize: "1.875rem", marginBottom: 40 }}>
            Page not found
          </div>
          <div style={{ fontSize: "1.125rem" }}>
            Page not foundPlease contact us on our Discord server if you think you
            shouldn't see this page!
          </div>
        </ErrorContainer>
      ),
      {
