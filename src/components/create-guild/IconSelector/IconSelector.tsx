import {
  FormControl,
  FormLabel, IconButton, ModalBody,
  ModalCloseButton,
  ModalContent,
  ModalHeader,
  ModalOverlay,
  SimpleGrid, Tab, TabList, TabPanel, TabPanels, Tabs, useDisclosure, useRadioGroup
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <FormControl>
              ...
              <Tabs isFitted  variant='enclosed' defaultIndex={tabIndex} colorScheme='white' onChange={(index) => setTabIndex(index)}>
                <TabList> 
                {icons.map((tab, index) => {
                  const radio = getRadioProps({
                    value: `/guildLogos/${tab.logo}.svg`,
                  })
                  return <Tab key={index}>{index == tabIndex ? tab.name : ''}<img src={`/guildLogos/${tab.logo}.svg`} /></Tab>
                })}
                </TabList>
                <TabPanels>
                  {icons.map((tab, index) => (
